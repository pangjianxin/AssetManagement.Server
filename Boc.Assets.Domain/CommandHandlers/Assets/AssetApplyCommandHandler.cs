...
namespace Boc.Assets.Domain.CommandHandlers.Assets
{
        ...
        IRequestHandler<RemoveAssetApplyCommand, bool>
    {
        ...
        public async Task<bool> Handle(ApplyAssetCommand request, CancellationToken cancellationToken)
        {
            ...
            {
                await _bus.RaiseEventAsync(new DomainNotification("系统错误", "传入的资产分类参数有误，请联系管理员"));
                return false;
            }
            var targetOrg = await _organizationRepository.GetByIdAsync(request.TargetOrgId);
            if (targetOrg == null)
            {
                await _bus.RaiseEventAsync(new DomainNotification("系统错误", "查找目标机构信息为空，请联系管理员"));
                return false;
            }

            var assetApply = new AssetApply(request.Principal,
                targetOrg,
                assetCategory.Id,
            ...
            {
                await _bus.RaiseEventAsync(new AssetApplyEvent(assetApply));
                return true;
            }
            return false;
        }
        public async Task<bool> Handle(HandleAssetApplyCommand request, CancellationToken cancellationToken)
        {
            ...
            {
                await _bus.RaiseEventAsync(new DomainNotification("状态错误", "传入的资产状态不为在库，不能分配该资产"));
                return false;
            }
            ...
            if (await CommitAsync())
            {
                ...
                await _bus.RaiseEventAsync(new AssetApplyHandledEvent(assetApply));
                return true;
            }
            return false;
        }
        public async Task<bool> Handle(RevokeAssetApplyCommand request, CancellationToken cancellationToken)
        {
            ...
            if (await CommitAsync())
            {
                await _bus.RaiseEventAsync(new AssetApplyRevokedEvent(assetApply, request.Message));
                return true;
            }
            return false;
        }
        ...
        public async Task<bool> Handle(RemoveAssetApplyCommand request, CancellationToken cancellationToken)
        {
            ...
            if (assetApply == null)
            {
                await _bus.RaiseEventAsync(new DomainNotification("系统错误", "为找到指定的事件进行处理，请联系管理员"));
                return false;
            }
